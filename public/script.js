  ...
  try {
    if (!listing.imageUrl && listing.link) {
      ...
      if (fetchedImageUrl) {
        ...
        console.warn(
          `No image found for listing ${index + 1}: ${listing.title}`
        );
      }
    }
  } catch (error) {
    console.error(
      `Error fetching image for listing ${index + 1} (${listing.title}):`,
      error
    );
  }
}
...
async function displayResults(listings) {
  ...
  for (const listing of listings) {
    ...
    row.innerHTML = `
            ...
            <td>${price}</td>
            <td><img src="${listing.imageUrl || "placeholder.png"}" alt="${
      listing.title
    }" /></td>
  ...
  if (navigator.geolocation) {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        const apiKey =
          "5b3ce3597851110001cf62482e73e166785f414da55420c7c4ef8591";
        // Call ORS API to get location details
        fetch(
          `https://api.openrouteservice.org/geocode/reverse?api_key=${apiKey}&point.lon=${longitude}&point.lat=${latitude}`
        )
...
function updateAds() {
  ...
  if (state === "hide") {
    ...
    allAds.forEach((ad) => {
      ...
      ad.style.overflow = ad.classList.contains("highlighted-ad")
        ? "visible"
        : "hidden";
    });
